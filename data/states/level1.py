...
class Level1(tools._State):
    def __init__(self):
        ...
        self.load_key_pressed = False
        self.screenshot_key_pressed = False
        self.help_key_pressed = False
        self.cheat_toggle_pressed = False
        self.lives_cheat_pressed = False
        self.coins_cheat_pressed = False
        self.score_cheat_pressed = False
        self.invincible_cheat_pressed = False
    ...
    def update(self, surface, keys, current_time):
        ...
        self.handle_key_inputs(keys, surface)
        
        # Pause game when help screen is shown
        ...
        if self.show_help:
            self.draw_help_screen(surface)
        self.update_message_display()
        self.draw_message(surface)



    def handle_key_inputs(self, keys, surface):
        """Handle special key inputs for save, load, screenshot, help, and cheats"""
        # Save game (F5)
        if keys[pg.K_F5] and not self.save_key_pressed:
            self.save_key_pressed = True
            self.save_game_progress()
        elif not keys[pg.K_F5]:
            self.save_key_pressed = False
        
        # Load game (F9)
        if keys[pg.K_F9] and not self.load_key_pressed:
            self.load_key_pressed = True
            self.load_game_progress()
        elif not keys[pg.K_F9]:
            self.load_key_pressed = False
        
        # Screenshot (F12)
        if keys[pg.K_F12] and not self.screenshot_key_pressed:
            self.screenshot_key_pressed = True
            self.take_screenshot(surface)
        elif not keys[pg.K_F12]:
            self.screenshot_key_pressed = False
        
        # Help screen (P)
        if keys[pg.K_p] and not self.help_key_pressed:
            self.help_key_pressed = True
            self.show_help = not self.show_help
            if self.show_help:
                self.help_timer = self.current_time
        elif not keys[pg.K_p]:
            self.help_key_pressed = False
        
        # Cheat mode toggle (F1)
        if keys[pg.K_F1] and not self.cheat_toggle_pressed:
            self.cheat_mode = not self.cheat_mode
            self.cheat_toggle_pressed = True
            if self.cheat_mode:
                self.show_message("CHEAT MODE ENABLED", c.GREEN)
            else:
                self.show_message("CHEAT MODE DISABLED", c.YELLOW)
        elif not keys[pg.K_F1]:
            self.cheat_toggle_pressed = False
        
        # Apply cheats if cheat mode is enabled
        if self.cheat_mode:
            self.handle_cheats(keys)
    
    def handle_cheats(self, keys):
        """Handle cheat key combinations"""
        try:
            # Invincibility (F2)
            if keys[pg.K_F2] and not self.invincible_cheat_pressed:
                if hasattr(self.mario, 'invincible'):
                    self.mario.invincible = True
                    if hasattr(self.mario, 'invincible_start_timer'):
                        self.mario.invincible_start_timer = self.current_time
                    self.show_message("INVINCIBILITY ACTIVATED", c.CYAN)
                self.invincible_cheat_pressed = True
            elif not keys[pg.K_F2]:
                self.invincible_cheat_pressed = False
            
            # Big Mario (F3)
            if keys[pg.K_F3] and hasattr(self.mario, 'big') and not self.mario.big:
                if hasattr(self.mario, 'in_transition_state') and not self.mario.in_transition_state:
                    if hasattr(self.mario, 'state'):
                        self.mario.state = c.SMALL_TO_BIG
                        self.mario.in_transition_state = True
                        if hasattr(self.mario, 'transition_timer'):
                            self.mario.transition_timer = 0
                        self.show_message("BECOMING BIG MARIO", c.GREEN)
            
            # Fire Mario (F4)
            if keys[pg.K_F4] and hasattr(self.mario, 'big') and self.mario.big:
                if hasattr(self.mario, 'fire') and not self.mario.fire:
                    if hasattr(self.mario, 'in_transition_state') and not self.mario.in_transition_state:
                        if hasattr(self.mario, 'state'):
                            self.mario.state = c.BIG_TO_FIRE
                            self.mario.in_transition_state = True
                            if hasattr(self.mario, 'fire_transition_timer'):
                                self.mario.fire_transition_timer = 0
                            self.show_message("BECOMING FIRE MARIO", c.RED)
            
            # Add lives (F6)
            if keys[pg.K_F6] and not self.lives_cheat_pressed:
                if c.LIVES in self.game_info:
                    self.game_info[c.LIVES] += 1
                    self.show_message("+1 LIFE", c.GREEN)
                self.lives_cheat_pressed = True
            elif not keys[pg.K_F6]:
                self.lives_cheat_pressed = False
            
            # Add coins (F7)
            if keys[pg.K_F7] and not self.coins_cheat_pressed:
                if c.COIN_TOTAL in self.game_info:
                    self.game_info[c.COIN_TOTAL] = min(99, self.game_info[c.COIN_TOTAL] + 10)
                    self.show_message("+10 COINS", c.YELLOW)
                self.coins_cheat_pressed = True
            elif not keys[pg.K_F7]:
                self.coins_cheat_pressed = False
            
            # Add score (F8)
            if keys[pg.K_F8] and not self.score_cheat_pressed:
                if c.SCORE in self.game_info:
                    self.game_info[c.SCORE] += 1000
                    self.show_message("+1000 SCORE", c.GOLD)
                self.score_cheat_pressed = True
            elif not keys[pg.K_F8]:
                self.score_cheat_pressed = False
        except Exception as e:
    ...
    def save_game_progress(self):
        ...
        try:
            mario_state = {
                ...
                'x_vel': self.mario.x_vel,
                'y_vel': self.mario.y_vel
            }
            
            viewport_x = self.viewport.x
            
            # Save time from overhead info display
            game_info_copy = self.game_info.copy()
            if hasattr(self.overhead_info_display, 'time'):
                game_info_copy['time'] = self.overhead_info_display.time
            
            if save_load.save_game(game_info_copy, mario_state, viewport_x):
                self.show_message("GAME SAVED SUCCESSFULLY", c.GREEN)
            else:
                self.show_message("FAILED TO SAVE GAME", c.RED)
        except Exception as e:
            self.show_message(f"SAVE ERROR: {str(e)[:30]}", c.RED)
            print(f"Error saving game: {e}")
    
    def load_game_progress(self):
        """Load saved game progress"""
        try:
            game_info, mario_state, viewport_x = save_load.load_game()
            
            if game_info is None:
                self.show_message("NO SAVE FILE FOUND", c.RED)
                return
            ...
            # Restore Mario state
            if mario_state and hasattr(self, 'mario'):
                try:
                    if 'x' in mario_state:
                        self.mario.rect.x = mario_state['x']
                    if 'y' in mario_state:
                        self.mario.rect.y = mario_state['y']
                    if 'big' in mario_state:
                        self.mario.big = mario_state['big']
                    if 'fire' in mario_state:
                        self.mario.fire = mario_state['fire']
                    if 'invincible' in mario_state:
                        self.mario.invincible = mario_state['invincible']
                    if 'state' in mario_state:
                        self.mario.state = mario_state['state']
                    if 'facing_right' in mario_state:
                        self.mario.facing_right = mario_state['facing_right']
                    if 'x_vel' in mario_state:
                        self.mario.x_vel = mario_state['x_vel']
                    if 'y_vel' in mario_state:
                        self.mario.y_vel = mario_state['y_vel']
                    
                    # Update Mario's appearance based on state
                    if hasattr(self.mario, 'big') and self.mario.big:
                        if hasattr(self.mario, 'become_big'):
                            self.mario.become_big()
                    if hasattr(self.mario, 'fire') and self.mario.fire:
                        if hasattr(self.mario, 'big') and self.mario.big:
                            if hasattr(self.mario, 'fire_frames'):
                                self.mario.right_frames = self.mario.fire_frames[0]
                                self.mario.left_frames = self.mario.fire_frames[1]
                except Exception as e:
                    print(f"Error restoring Mario state: {e}")
            
            # Update overhead info
            if 'time' in self.game_info:
                self.overhead_info_display.time = self.game_info['time']
                self.overhead_info_display.current_time = self.current_time
            
            self.show_message("GAME LOADED SUCCESSFULLY", c.GREEN)
        except Exception as e:
            self.show_message(f"LOAD ERROR: {str(e)[:30]}", c.RED)
    ...
    def take_screenshot(self, surface):
        ...
        try:
            ...
            self.show_message("SCREENSHOT SAVED", c.CYAN)
        except Exception as e:
            self.show_message("SCREENSHOT FAILED", c.RED)
            print(f"Error taking screenshot: {e}")
    
    def draw_help_screen(self, surface):
        ...
        try:
            ...
            # Create font for help text - use default font
            try:
                font_large = pg.font.Font(None, 48)
                font_medium = pg.font.Font(None, 28)
                font_small = pg.font.Font(None, 22)
            except:
                # Fallback to default font if None doesn't work
                font_large = pg.font.SysFont('arial', 48)
                font_medium = pg.font.SysFont('arial', 28)
                font_small = pg.font.SysFont('arial', 22)
            
            # Title
            title = font_large.render("GAME CONTROLS & FEATURES", True, c.WHITE)
            title_rect = title.get_rect(center=(c.SCREEN_WIDTH // 2, 40))
            surface.blit(title, title_rect)
            
            # Help text - split into two columns
            left_column = [
                "BASIC CONTROLS:",
                "  W - Jump",
                "  A - Move Left",
            ...
            # Draw left column - positioned at 1/6 of screen width (more to the left)
            left_x = c.SCREEN_WIDTH // 6
            y_offset = 100
            
            for line in left_column:
                if line.startswith("  "):
                    text = font_small.render(line, True, c.YELLOW)
                elif line == "":
                    y_offset += 8
                    continue
                elif line.endswith(":"):
                    text = font_medium.render(line, True, c.CYAN)
                else:
                    text = font_small.render(line, True, c.WHITE)
                
                text_rect = text.get_rect(left=left_x, top=y_offset)
                surface.blit(text, text_rect)
                y_offset += 26
            
            # Draw right column - positioned at 4/5 of screen width (more to the right, larger gap)
            right_x = c.SCREEN_WIDTH * 4 // 5
            y_offset = 100
            ...
            for line in right_column:
                ...
                text_rect = text.get_rect(right=right_x, top=y_offset)
                surface.blit(text, text_rect)
                y_offset += 26
            
            # Draw instruction at bottom
            instruction = font_medium.render("Press P to resume game", True, c.WHITE)
            instruction_rect = instruction.get_rect(center=(c.SCREEN_WIDTH // 2, c.SCREEN_HEIGHT - 40))
            surface.blit(instruction, instruction_rect)
            
        except Exception as e:
            print(f"Error drawing help screen: {e}")
    
    def show_message(self, text, color=c.WHITE):
        """Display a message on screen"""
        self.message_text = text
        self.message_color = color
        self.message_timer = self.current_time
    
    def update_message_display(self):
        """Update message display timer"""
        if self.message_text and self.message_timer > 0:
            if (self.current_time - self.message_timer) > self.message_duration:
                self.message_text = ""
                self.message_timer = 0
    ...
    def draw_message(self, surface):
        ...
        if self.message_text and self.message_timer > 0:
            try:
                ...
                text_color = getattr(self, 'message_color', c.WHITE)
                
                # Create text with shadow/outline
                text_surface = font.render(self.message_text, True, text_color)
                shadow_surface = font.render(self.message_text, True, c.BLACK)
                
                # Calculate position (center top)
                text_rect = text_surface.get_rect(center=(c.SCREEN_WIDTH // 2, 100))
                shadow_rect = shadow_surface.get_rect(center=(c.SCREEN_WIDTH // 2 + 2, 102))
